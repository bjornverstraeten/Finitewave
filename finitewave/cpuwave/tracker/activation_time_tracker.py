...
class ActivationTimeTracker(Tracker):
    """
    A class to track and record the activation time of each cell in a
    ...
    ----------
    act_t : np.ndarray
        Array to store the activation time of each cell in the model grid.
    threshold : float
    ...
    def __init__(self):
        ...
        self.act_t = None            # Array to store activation times
        self.threshold = -40         # Threshold for activation (in mV)
        self.file_name = "act_time"  # Default file name for saving data
